...
describe('Hardened Security Acceptance Tests', () => {
    ...
    test('should reject valid JWT but expired session in DB', async () => {
        ...
        const token = jwt.sign({ user: testUser, sessionId: session.id }, JWT_SECRET);
    ...
    test('should reject session bound to a different user', async () => {
        ...
        const token = jwt.sign({ user: testUser, sessionId: sessionOfOther.id }, JWT_SECRET);
        const { req, res } = createMocks({
            method: 'GET',
            headers: { authorization: `Bearer ${token}` }
        });
    ...
    test('should allow multiple concurrent sessions for the same user', async () => {
        ...
        const t1 = jwt.sign({ user: testUser, sessionId: s1.id }, JWT_SECRET);
        const t2 = jwt.sign({ user: testUser, sessionId: s2.id }, JWT_SECRET);
